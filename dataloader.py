...
class DeepSeqDataset(Dataset):
	...
	def __getitem__(self, idx):
		file = self.data_48[idx]
		folder_name = file['vid_name']
		label = file['label']
		action = vocab[label,:].astype(np.int32)

		folder_path = self.frames_dir+folder_name

		revert= np.array(range(self.seq_len-1,-1,-1))
		vid_frames = np.zeros((self.seq_len,3,224,224),dtype=np.float32)
		
		num_images = len(os.listdir(folder_path))
		a = np.random.uniform(0,0.99999,seq_len)
